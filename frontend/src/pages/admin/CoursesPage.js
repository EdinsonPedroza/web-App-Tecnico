...
export default function CoursesPage() {
  ...
  const openEdit = (course) => {
    ...
    setIsAutoGeneratedName(false);
    setModuleDateErrors({});
    setDialogProgramId(course.program_id);
    // Load enrolled students separately so they always appear in the list regardless of search.
    // page_size=200 covers typical course enrollment (courses rarely exceed 200 enrolled students).
    if (course.student_ids && course.student_ids.length > 0) {
      api.get(`/users?role=estudiante&program_id=${course.program_id}&page_size=200`)
        .then(res => setEnrolledStudentsData(res.data.users || []))
        .catch(() => setEnrolledStudentsData([]));
    } else {
      setEnrolledStudentsData([]);
    }
    loadStudentsForDialog(course.program_id, '');
    setDialogOpen(true);
  };
