...
export default function CoursesPage() {
  ...
  // Debounce studentSearch → studentSearchDebounced (400ms)
  useEffect(() => {
    const timer = setTimeout(() => setStudentSearchDebounced(studentSearch), 400);
  ...
  const fetchData = useCallback(async () => {
    try {
      const [cRes, pRes, sRes, tRes] = await Promise.all([
        api.get('/courses?fields=summary'), api.get('/programs'), api.get('/subjects'),
  ...
  const loadStudentsForDialog = useCallback(async (programId, searchTerm = '') => {
    if (!programId) { setStudents([]); setStudentsTotal(0); return; }
    try {
      const res = await api.get(`/users?role=estudiante&program_id=${programId}&page_size=50&search=${encodeURIComponent(searchTerm)}`);
  ...
  // Reload student search results when debounced search term changes (dialog must be open with a program)
  const [dialogProgramId, setDialogProgramId] = useState('');
  useEffect(() => {
    if (dialogOpen && dialogProgramId) {
      loadStudentsForDialog(dialogProgramId, studentSearchDebounced);
    }
  }, [studentSearchDebounced, dialogOpen, dialogProgramId, loadStudentsForDialog]);
  ...
  const openEdit = async (course) => {
    // Cargar el curso completo (la lista usa fields=summary que omite student_ids)
    ...
    setEditing(fullCourse);
    // Extract month and year from course name if possible
    const monthMatch = fullCourse.name.match(/(Enero|Febrero|Marzo|Abril|Mayo|Junio|Julio|Agosto|Septiembre|Octubre|Noviembre|Diciembre)/i);
    const yearMatch = fullCourse.name.match(/\d{4}/);
    setForm({ 
      name: fullCourse.name, 
      program_id: fullCourse.program_id, 
      subject_id: fullCourse.subject_id || '',
      subject_ids: fullCourse.subject_ids || (fullCourse.subject_id ? [fullCourse.subject_id] : []),
      year: yearMatch ? parseInt(yearMatch[0]) : new Date().getFullYear(),
      month: monthMatch ? monthMatch[0] : 'Enero',
      student_ids: fullCourse.student_ids || [],
      start_date: fullCourse.start_date || '',
      end_date: fullCourse.end_date || '',
      module_dates: fullCourse.module_dates || {}
    });
    setSubjectSearch('');
    setStudentSearch('');
    setStudentSearchDebounced('');
    setIsAutoGeneratedName(false);
    setModuleDateErrors({});
    setDialogProgramId(fullCourse.program_id);
    if (fullCourse.student_ids && fullCourse.student_ids.length > 0) {
      api.get(`/users?role=estudiante&program_id=${fullCourse.program_id}&page_size=200`)
        .then(res => setEnrolledStudentsData(res.data.users || []))
        .catch(() => setEnrolledStudentsData([]));
    } else {
      setEnrolledStudentsData([]);
    }
    loadStudentsForDialog(fullCourse.program_id, '');
    setDialogOpen(true);
  };
                  ...
                  <Select value={form.program_id} onValueChange={(v) => {
                    if (form.subject_ids.length > 0) {
                      if (window.confirm('¿Cambiar el programa borrará las materias seleccionadas. Continuar?')) {
                        ...
                        setStudentSearch('');
                        setStudentSearchDebounced('');
                        setDialogProgramId(v);
                        loadStudentsForDialog(v, '');
                      }
                    } else {
                      setForm({ ...form, program_id: v, subject_ids: [] });
                      setStudentSearch('');
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                {(() => {
                  ...
                  // Merge enrolled students with search results for button computation
                  const enrolledIds = new Set(form.student_ids);
                  const mergedForBtn = [
                    ...(enrolledStudentsData.filter(s => enrolledIds.has(s.id))),
                  ...
                  const eligible = mergedForBtn.filter(s => {
              ...
              {studentsTotal > 0 && (
                <p className="text-xs text-muted-foreground px-1">
                  Mostrando {students.length} de {studentsTotal} estudiantes. Usa el buscador para encontrar más.
                </p>
              )}
              <div className="max-h-48 overflow-y-auto rounded-lg border p-4 space-y-2.5">
                {(() => {
                  ...
                  // Merge enrolled students (always visible) with server search results (de-duped)
                  const enrolledIds = new Set(form.student_ids);
                  ...
                  // Students removed from THIS course due to failed recovery cannot re-join it
                  const removedFromThisCourse = editing ? (editing.removed_student_ids || []) : [];
                  const eligible = mergedStudents.filter(s => {
                    // Always include students already enrolled in the current group
                    if (editing && form.student_ids.includes(s.id)) return true;
                    // Exclude students previously removed from this course due to failed recovery
                    if (removedFromThisCourse.includes(s.id)) return false;
                  ...
                  if (mergedStudents.length === 0) return <p className="text-sm text-muted-foreground">No hay estudiantes</p>;
